...
from torch_geometric.nn import GATv2Conv  # 使用GATv2Conv更现代的实现
from torch_geometric.data import Data, Batch
import json
import torch.nn.functional as F

class GAT(nn.Module):
    def __init__(self, in_channels=384, out_channels=1024):  # 384是MiniLM-L12-v2的输出维度
        super().__init__()
        
        self.gat_layers = nn.ModuleList([
            # 第一层: in_channels -> 1024
            GATv2Conv(
            ...
            # 最后一层: 4096 (1024*4) -> 1024，不concat heads
            GATv2Conv(
                in_channels=1024 * 4,
                out_channels=out_channels,
                heads=4,
                concat=False,  # 最后一层不concat
                dropout=0.1
            )
        ])
    ...
    def forward(self, x, edge_index):
        ...
        # 最后一层: concat=False
        x = self.gat_layers[3](x, edge_index)
        return x
...
class InstructBlipForConditionalGeneration(InstructBlipPreTrainedModel):
    ...
    def __init__(self, config: InstructBlipConfig):
        super().__init__(config)
        ############## 自己加 ###########################
        self.gat = GAT(out_channels=config.text_config.hidden_size)
        self.out_channel = config.text_config.hidden_size
        ##### 创建embedding model，用来编码图的文本
        self.embed_tokenizer = AutoTokenizer.from_pretrained('sentence-transformers/all-MiniLM-L12-v2')
        self.embedding_model = AutoModel.from_pretrained('sentence-transformers/all-MiniLM-L12-v2')
        #################################################
    ...
    def encode_graph(self, graph_str, dtype):
        # 1. 首先计算最大节点数和预处理图
        max_nodes = 0
        processed_graphs = []
        for graph in graph_str:
            graph_dict = json.loads(graph)
            nodes = graph_dict["nodes"]
            max_nodes = max(max_nodes, len(nodes) if len(nodes) > 0 else 1)
            processed_graphs.append(graph_dict)
        
        # 2. 处理每个图，填充到最大节点数
        graph_data_list = []
        for graph_dict in processed_graphs:
            nodes = graph_dict["nodes"]
            ...
            if len(nodes) == 0:
                graph_data = Data(
                    x=torch.zeros((max_nodes, 384), dtype=dtype, device=self.device),
            ...
            # 编码节点
            with torch.no_grad():
                encoded_input = self.embed_tokenizer(nodes, padding=True, truncation=True, return_tensors='pt').to(self.device)
                ...
                node_embeddings = self.embedding_model(**encoded_input)
                # Perform pooling
                sentence_embeddings = self.mean_pooling(node_embeddings, encoded_input['attention_mask'])
                # Normalize embeddings
                sentence_embeddings = F.normalize(sentence_embeddings, p=2, dim=1)
                node_embeddings = sentence_embeddings.to(dtype=dtype)
                
            # 填充到最大节点数
            padded_embeddings = torch.zeros((max_nodes, 384), dtype=dtype, device=self.device)
            padded_embeddings[:len(nodes)] = node_embeddings
        ...
        batch.x = batch.x.to(dtype=dtype, device=self.device)
        batch.edge_index = batch.edge_index.to(device=self.device)
        
        # 4. GAT处理
        out = self.gat(batch.x, batch.edge_index)
        
        # 5. 重塑输出为 [batch_size, max_nodes, features]
        batch_size = len(graph_data_list)
        # print(out.shape)
        out = out.view(batch_size, max_nodes, self.out_channel)
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        interpolate_pos_encoding: bool = False,
        graph_str: Optional[str] = None,  # 新增参数######################
        ...
        ##### 编码图
        graph_embeddings = self.encode_graph(graph_str, dtype)  # [b, N, hidden_size]
        ...
        inputs_embeds = torch.cat([graph_embeddings, language_model_inputs, inputs_embeds.to(language_model_inputs.device)], dim=1)

        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        attention_mask = torch.cat([graph_attention_mask, language_model_attention_mask.to(attention_mask.device), attention_mask], dim=1)
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        attention_mask = torch.cat([graph_attention_mask, language_attention_mask, attention_mask.to(language_attention_mask.device)], dim=1)
